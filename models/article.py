...
class ArticleModel:
    ...
    def modify_article(self, art_code, new_data):
        result = self.collection.update_one({Schema.CODE: art_code}, {"$set": new_data})
        return result.modified_count > 0

    def delete_article(self, art_code):
        self.collection.delete_one({Schema.CODE: art_code})
    ...
    def get_article_dems(self, art_id):
        ...
        articles = list(self.collection.find({Schema.CODE: code}))
        dems = []
        P = Schema.Product
        for article in articles:
            # Vérifier si l'article a un champ produits
            produits = get_field_value(article, Schema.PRODUCTS, "produits")
            if produits:
                for prod in produits:
                    dem = get_field_value(prod, P.DEM, "DEM", "dem")
                    if dem:
                        dems.append(dem)
            # Vérifier si l'article a un champ dem direct (legacy)
            direct_dem = get_field_value(article, P.DEM, "dem")
            if direct_dem:
                dems.append(direct_dem)
    ...
    def update_product_details(article_code, product_index, quantity_to_deduct):
        ...
        try:
            ...
            current_quantity = float(get_field_value(product, P.QUANTITY, "Quantité", "quantite", default=0))
            ...
            product[P.QUANTITY] = str(new_quantity)  # Update the product quantity

            # Recalculate the main quantity as the sum of all product quantities
            total_quantity = sum(float(get_field_value(p, P.QUANTITY, "Quantité", "quantite", default=0)) for p in products)

            # Update the article in the database
            db.articles.update_one(
                {Schema.CODE: article_code},
                {"$set": {Schema.PRODUCTS: products, Schema.QUANTITY: total_quantity}}
            )
    ...
    def recalculate_main_quantity(article_code):
        ...
        try:
            P = Schema.Product
            total_quantity = sum(float(get_field_value(product, P.QUANTITY, "Quantité", "quantite", default=0)) for product in products)

            # Update the main quantity in the database
            db.articles.update_one(
                {Schema.CODE: article_code},
                {"$set": {Schema.QUANTITY: total_quantity}}
            )
